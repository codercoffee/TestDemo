...
- (BOOL)exportRSAKeys
{
    ...
    if (_rsa != NULL) {
        ...
        filepri = fopen([OpenSSLRSAPrivateKeyFile cStringUsingEncoding:NSUTF8StringEncoding],"w");
        filepub = fopen([OpenSSLRSAPublicKeyFile cStringUsingEncoding:NSUTF8StringEncoding],"w");
...
- (NSData *)encryptRSAKeyWithType:(KeyType)keyType plainText:(NSString*)text
{
    if (text && [text length])
    {
        ...
        //unsigned char * encData = (unsigned char *)malloc(flen);
        bzero(encData, flen);
        ...
        if (status)
        {
            ...
            //            NSData * da = [NSData dataWithBytes:encData length:strlen(encData)];
            //            NSLog(@"===%@",da);
            ...
            //NSString *filePath = [NSString stringWithFormat:@"%@/Documents/3.txt",NSHomeDirectory()];
            //[returnData writeToFile:filePath atomically:YES];
            
            
            //NSString * str = [returnData base64EncodedString];
            //NSString * str = [returnData description];
            //NSLog(@"******=======%@",str);
